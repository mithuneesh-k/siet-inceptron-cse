...
db.defaults({ users: [], achievements: [], teams: [], team_members: [] }).write();

let _uid = db.get('users').value().reduce((m, u) => Math.max(m, u.id), 0);
let _aid = db.get('achievements').value().reduce((m, a) => Math.max(m, a.id), 0);
let _tid = db.get('teams').value().reduce((m, t) => Math.max(m, t.id), 0);
...
if (db.get('users').size().value() === 0) {
  console.log('🌱 Seeding database with real 2025-batch student data...');
  ...
  db.get('users').push({
    ...
    bio: 'Head of Department, CSE — Sri Shakthi Institute of Engineering and Technology, Coimbatore.',
    avatar_url: null, is_admin: true, created_at: now
  }).write();

  // ── Real 2025-batch students ──
  // Class mapping: 25CSEA→CSE-A, 25CSEB→CSE-B, 25CSEC→CSE-C, 25CSED→CSE-D, 25CSEE→CSE-E
  ...
  rawStudents.forEach(([rollNo, regNo, name, rawClass], i) => {
    const uid = i + 2; // IDs start at 2 (1 = admin)
    const cls = classMap[rawClass] || rawClass;
    // Email: roll number lowercase @SIET.ac.in
    const email = `${rollNo.toLowerCase()}@SIET.ac.in`;
    db.get('users').push({
      id: uid,
      name: name.trim(),
      roll_no: rollNo,
      reg_no: regNo,
      year: 1,  // 2025 batch = 1st year
      class: cls,
      email,
      password_hash: pass,
      github: null,
  ...
  ids.user.current = rawStudents.length + 1;

  console.log(`✅ Seeded ${rawStudents.length} real students from 2025 batch!`);
  console.log('   Admin:   admin@SIET.ac.in  / password123');
  console.log('   Student: 25cs001@SIET.ac.in / password123  (AADHIL YUNIS U)');
  console.log('   Student: 25cs144@SIET.ac.in / password123  (MITHUNEESH K)');
  console.log('');
  console.log('   Classes: CSE-A (001–064) | CSE-B (065–125) | CSE-C (126–188) | CSE-D (189–250) | CSE-E (251–313)');
}

// ─── Helper ─────────────────────────────────────
function getUserWithScore(id) {
  const user = db.get('users').find({ id: parseInt(id) }).value();
  if (!user) return null;
  const { password_hash, ...safeUser } = user;
  const achs = db.get('achievements').filter({ user_id: parseInt(id), verified: true }).value();
  safeUser.score = achs.reduce((s, a) => s + a.points, 0);
  safeUser.achievement_count = achs.length;
  return safeUser;
}

module.exports = { db, calcPoints, ids, getUserWithScore };
