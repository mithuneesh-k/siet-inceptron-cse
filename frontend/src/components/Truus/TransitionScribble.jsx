...
export default function TransitionScribble() {
    useEffect(() => {
        const logoInceptronClickable = document.querySelector('.logo-inceptron');
        ...
        if (!logoInceptronClickable || !transitionScribblePath || !transitionScribbleSvg) return;
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                const svgClone = document.querySelector('.logo-inceptron').cloneNode(true);
        ...
        logoInceptronClickable.addEventListener('click', runScribbleAnimation);

        // Auto-run on load
        const timer = setTimeout(() => runScribbleAnimation(null), 100);

        return () => {
            logoInceptronClickable.removeEventListener('click', runScribbleAnimation);
            clearTimeout(timer);
        };
    }, []);
